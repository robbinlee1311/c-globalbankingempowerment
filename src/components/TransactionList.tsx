        ...
        {transactions.map((tx) => (
          <div key={tx.id} className='flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg transition'>
            <div className='flex items-center gap-3'>
              <div className={p-2 rounded-full }>{tx.type === 'credit' ? <ArrowDownLeft className='w-4 h-4' /> : <ArrowUpRight className='w-4 h-4' />}</div>
            ...
            <span className={ont-semibold }>{tx.type === 'credit' ? '+' : '-'}</span>
          </div>
        ))}
      </div>
    </div>
