    ...
    <div className='relative'>
      ...
      {open && (
        <div className='absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-2xl border z-50'>
          <div className='p-4 border-b flex justify-between items-center'><h4 className='font-bold text-gray-800'>Notifications</h4><button onClick={() => setOpen(false)}><X className='w-4 h-4 text-gray-500' /></button></div>
          {permission !== 'granted' && <div className='p-3 bg-blue-50 text-xs'><button onClick={requestBrowserNotify} className='text-blue-600 font-semibold hover:underline'>Enable browser push notifications</button></div>}
          <div className='max-h-64 overflow-y-auto'>
            {notifications.map(n => (
              <div key={n.id} className={p-3 border-b hover:bg-gray-50 }><p className='text-sm text-gray-800'>{n.text}</p><p className='text-xs text-gray-500 mt-1'>{n.time}</p></div>
            ))}
          </div>
        </div>
      )}
