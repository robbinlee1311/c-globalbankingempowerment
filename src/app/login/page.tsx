  ...
  return (
    <div className='min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-900 to-blue-700'>
      <div className='bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md'>
        ...
        <p className='text-center mt-4 text-sm text-gray-600'>{isSignUp ? 'Already have an account?' : 'Don	 have an account?'} <button onClick={() => setIsSignUp(!isSignUp)} className='text-blue-600 font-semibold hover:underline'>{isSignUp ? 'Sign In' : 'Sign Up'}</button></p>
      </div>
    </div>
  )
}
